...
int main() {
  ...
  if (flex_is_first_core() &&
      CID == 0) { // only allow core 0 in cluster 0 to print
      ...
      if (flex_is_first_core()) {
        for (uint32_t buf = 0; buf < DeeployNetwork_num_inputs; buf++) {
          for (uint32_t i = 0; i < 4; i++) {
            ...
            printf("[main.c] >>> After DMA, first few elements of input "
                   "buffer %lu: %f\r\n",
                   buf, val);
          }
        }
		for (uint32_t buf = 0; buf < DeeployNetwork_num_outputs; buf++) {
		  for (uint32_t i = 0; i < 4; i++) {
			float val = fp16_to_float(((fp16 *)DeeployNetwork_outputs[buf])[i]);
			printf("[main.c] >>> After DMA, first few elements of output "
				   "buffer %lu: %f\r\n",
				   buf, val);
		  }
...
#define VERIFY_RTOL 1e-1f
    ...
    if (flex_is_first_core()) {
      for (uint32_t buf = 0; buf < DeeployNetwork_num_outputs; buf++) {
        ...
        uint32_t n_elems =
            DeeployNetwork_outputs_bytes[buf] / sizeof(OUTPUTTYPE);
        tot += n_elems;
